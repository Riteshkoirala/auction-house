<div class="modal fade" id="addLotModel-{{ $auction->id }}" tabindex="-1" aria-labelledby="addLotModelLabel{{ $auction->id }}" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="model-body p-3 text-dark">
                <form action="{{ route('lot.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <label>Title</label>
                    <input type="hidden" name="addLot" value="{{ $auction->id }}">
                    <input type="hidden" name="auction_id" value="{{ $auction->id }}">
                    <input type="text" class="form-control" name="title" value="{{ old('title') }}">
                    <p><span class="text-danger">@error('title') {{ $message }} @enderror</span></p>
                    <label>Lot Number</label>
                    <input type="number" class="form-control" name="lotNumber" value="{{ old('lotNumber') }}">
